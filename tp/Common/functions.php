...
function session($name,$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
		/*session初始化,特殊处理第三方接口session*/
		if(strtolower(GROUP_NAME) === 'third' && strtolower(MODULE_NAME) === 'fish'){
			C('VAR_SESSION_ID','fish_id');
		}elseif(strtolower(GROUP_NAME) === 'third' && strtolower(MODULE_NAME) !== 'fish'){
			C('VAR_SESSION_ID','hcmid');
		}
